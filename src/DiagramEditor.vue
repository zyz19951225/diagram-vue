...
export default {
  ...
  methods: {
      changeSvgUp(){
        this.$refs.Diagram.changeSvgUp()
      },
      changeSvgDown(){
          this.$refs.Diagram.changeSvgDown()
      },
      changeColor(time){
          this.$refs.Diagram.changeColor(time)
      },
      nowSelectNodeId(id){
        console.log(id)
      this.$emit("nowSelectNodeId",id)
      },
    ...
    addNode(item) {
      this.graphData.nodes.push({
        ...
        content: {
          ...
          color: item.content.color || 'white'
        },
        width: parseInt(item.width) || 250,
        height: parseInt(item.height) || 160,
        shape: item.shape,
        point: {
          x: 10,
